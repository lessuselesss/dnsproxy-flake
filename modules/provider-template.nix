...
let
  ...
  name = "<PROVIDER_NAME>"; # e.g., "google", "cloudflare"
  listenAddr = "<LISTEN_ADDR>"; # e.g., "127.8.8.88"
  description = "<DESCRIPTION>"; # e.g., "DNS Proxy for Google"
  
  # Plain DNS configuration
  plainDns = {
    enabled = true; # Set to false if plain DNS is not available
    upstreams = [
      "<PRIMARY_DNS>"   # e.g., "8.8.8.8"
      "<SECONDARY_DNS>" # e.g., "8.8.4.4"
    ];
  };
  
  # DoT (DNS over TLS) configuration
  dot = {
    enabled = false; # Set to true if DoT is available
    upstream = "<DOT_UPSTREAM>"; # e.g., "tls://dns.google"
  };
  
  # DoH (DNS over HTTPS) configuration
  doh = {
    enabled = false; # Set to true if DoH is available
    upstream = "<DOH_UPSTREAM>"; # e.g., "https://dns.google/dns-query"
  };
  
  # DoQ (DNS over QUIC) configuration
  doq = {
    enabled = false; # Set to true if DoQ is available
    upstream = "<DOQ_UPSTREAM>"; # e.g., "quic://dns.google"
  };
  
  # DNSCrypt configuration
  dnscrypt = {
    enabled = false; # Set to true if DNSCrypt is available
    stamp = "<DNSCRYPT_STAMP>"; # e.g., "sdns://..."
  };
